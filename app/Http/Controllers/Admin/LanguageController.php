...
class LanguageController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        // The default language owns the base content every other language
        // falls back to, so it cannot be removed.
        if ($language->code === base_locale()) {
            return redirect()->back()->withErrors(__('The default language cannot be deleted.'));
        }

        // Drop the content that belonged to this language so it does not sit
        // orphaned in the database.
        \App\Models\Translation::where('locale', $language->code)->delete();
        \App\Models\Settings::where('name', 'like', '%\_\_' . $language->code)->delete();

        if ($language->code !== base_locale() && file_exists(lang_path($language->code . '.json'))) {
