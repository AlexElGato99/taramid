    ...
    <main class="pt-[72px]">
        <section class="py-12 lg:py-20 bg-white">
            <div class="max-w-6xl mx-auto px-6 lg:px-8">
                ...
                <div class="mb-8">
                    <a href="<?php echo e(route('products.index')); ?>" class="inline-flex items-center gap-2 text-sm text-ash hover:text-ink transition-colors">
                ...
                <div class="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start" x-data="productGallery()">
                    ...
                    <div class="flex gap-4 lg:sticky lg:top-24 will-change-auto">
                        ...
                        <div class="hidden sm:flex flex-col gap-3 w-[72px] shrink-0">
                            <?php $__currentLoopData = $allImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <button @click="activeImage = <?php echo e($i); ?>"
                                        :class="activeImage === <?php echo e($i); ?> ? 'border-primary ring-1 ring-primary/20' : 'border-gray-200 hover:border-gray-300'"
                                        class="w-[72px] h-[72px] rounded-xl border-2 overflow-hidden transition-all duration-200 bg-surface">
                                    <img src="<?php echo e(asset('storage/' . $img)); ?>" alt="" class="w-full h-full object-cover">
                                </button>
                        ...
                        <div class="flex-1 aspect-[4/5] rounded-2xl overflow-hidden bg-surface relative">
                            <?php if(count($allImages)): ?>
                                <?php $__currentLoopData = $allImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <img x-show="activeImage === <?php echo e($i); ?>"
                                         x-transition:enter="transition ease-out duration-300"
                                         x-transition:enter-start="opacity-0"
                                         x-transition:enter-end="opacity-100"
                                         src="<?php echo e(asset('storage/' . $img)); ?>" alt="<?php echo e($product->t('title')); ?>"
                            ...
                            <?php if(count($allImages) > 1): ?>
                            <button @click="activeImage = activeImage > 0 ? activeImage - 1 : <?php echo e(count($allImages) - 1); ?>"
                            ...
                            <button @click="activeImage = activeImage < <?php echo e(count($allImages) - 1); ?> ? activeImage + 1 : 0"
                    ...
                    <div>
                        <h1 class="font-display text-3xl lg:text-4xl text-ink leading-tight mb-6"><?php echo e($product->t('title')); ?></h1>

                        <div class="space-y-0 border-t border-gray-100 mb-8" x-data="{ activeSection: null }">
                            <?php if($product->t('description')): ?>
                            <div class="border-b border-gray-100">
                                <button @click="activeSection = activeSection === 'description' ? null : 'description'" class="w-full flex items-center justify-between py-4 text-start">
                                    <span class="text-[15px] font-semibold text-ink"><?php echo e(__('Description')); ?></span>
                                ...
                                <div x-show="activeSection === 'description'" x-collapse>
                                    <div class="pb-4 text-sm text-ash leading-relaxed"><?php echo e($product->t('description')); ?></div>
                                </div>
                            </div>
                            <?php endif; ?>

                            <?php if($product->t('how_to_use')): ?>
                            <div class="border-b border-gray-100">
                                <button @click="activeSection = activeSection === 'how_to_use' ? null : 'how_to_use'" class="w-full flex items-center justify-between py-4 text-start">
                                    <span class="text-[15px] font-semibold text-ink"><?php echo e(__('How To Use')); ?></span>
                                ...
                                <div x-show="activeSection === 'how_to_use'" x-collapse>
                                    <div class="pb-4 text-sm text-ash leading-relaxed"><?php echo e($product->t('how_to_use')); ?></div>
                                </div>
                            </div>
                            <?php endif; ?>

                            <?php if($product->t('ingredients')): ?>
                            <div class="border-b border-gray-100">
                                <button @click="activeSection = activeSection === 'ingredients' ? null : 'ingredients'" class="w-full flex items-center justify-between py-4 text-start">
                                    <span class="text-[15px] font-semibold text-ink"><?php echo e(__('Ingredients')); ?></span>
                                ...
                                <div x-show="activeSection === 'ingredients'" x-collapse>
                                    <div class="pb-4 text-sm text-ash leading-relaxed"><?php echo e($product->t('ingredients')); ?></div>
                                </div>
                            </div>
                            <?php endif; ?>

                            <?php if($product->t('general_instructions')): ?>
                            <div class="border-b border-gray-100">
                                <button @click="activeSection = activeSection === 'instructions' ? null : 'instructions'" class="w-full flex items-center justify-between py-4 text-start">
                                    <span class="text-[15px] font-semibold text-ink"><?php echo e(__('General Instructions')); ?></span>
                                ...
                                <div x-show="activeSection === 'instructions'" x-collapse>
                                    <div class="pb-4 text-sm text-ash leading-relaxed"><?php echo e($product->t('general_instructions')); ?></div>
                                </div>
                            </div>
                            <?php endif; ?>
                        </div>

                        <p class="text-accent font-semibold text-lg mb-4"><?php echo e(__('Get a quotation')); ?></p>
                        <?php
                        ...
                        <div class="flex items-center justify-center gap-2 sm:gap-3 mb-6">
                            <?php $__currentLoopData = $actionButtons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $btn): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                    $icon = $btn['icon'] ?? 'link';
                                    $text = $btn['text'] ?? '';
                                    $value = $btn['value'] ?? '';
                                ...
                                ?>
                                <?php if($href && $text): ?>
                                    <a href="<?php echo e($href); ?>" target="<?php echo e($target); ?>" rel="noopener" class="<?php echo e($btnClass); ?> py-2.5 px-5 sm:py-3 sm:px-8 text-xs sm:text-sm whitespace-nowrap">
                                        <?php if($icon === 'email'): ?>
                        ...
                        <?php if($product->t('badge')): ?>
                            <span class="inline-flex items-center text-xs font-medium text-ash bg-surface px-3 py-1.5 rounded-full border border-gray-100"><?php echo e($product->t('badge')); ?></span>
                        <?php endif; ?>
        ...
        <section class="py-16 lg:py-24 bg-surface">
            <div class="max-w-6xl mx-auto px-6 lg:px-8">
                <h2 class="font-display text-2xl text-ink mb-10"><?php echo e(__('You may also like')); ?></h2>
                            ...
                            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-1">
                                <div class="aspect-square bg-surface relative overflow-hidden">
                                    <?php if($related->image): ?>
                                        <img src="<?php echo e(asset('storage/' . $related->image)); ?>" alt="<?php echo e($related->t('title')); ?>"
                                             class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">
                                    <?php endif; ?>
                                    <?php if($related->t('badge')): ?>
                                        <span class="absolute top-3 start-3 text-xs font-medium bg-white/90 backdrop-blur-sm text-ink px-3 py-1 rounded-full"><?php echo e($related->t('badge')); ?></span>
                                    <?php endif; ?>
                                </div>
                                <div class="p-4">
                                    <h3 class="font-display text-base text-ink mb-1 group-hover:text-primary transition-colors"><?php echo e($related->t('title')); ?></h3>
                                </div>
                            </div>
                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Dev_Projects\Laravel_Projects\taramid\projectfiles\allfilesbackupstaramid\resources\views/products/show.blade.php ENDPATH**/ ?>