...
<html lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>">
    <head>
        ...
        <title><?php if(isset($config['title'])): ?><?php echo e($config['title']); ?><?php else: ?><?php echo e(setting('title')); ?><?php endif; ?></title>
        <?php $favV = setting('favicon_version', '1'); $favType = setting('favicon_type', 'png'); ?>
        ...
        <link rel="manifest" href="<?php echo e(asset('site.webmanifest')); ?>?v=<?php echo e($favV); ?>">
        ...
        <style>
            ...
            <?php if(setting('palette')): ?>
                <?php $__currentLoopData = config('attr.colors.'.setting('palette')); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $color => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        <?php else: ?>
            <?php $__currentLoopData = config('attr.colors.zinc'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $color => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php endif; ?>
--color-primary-500: <?php if(setting('color')): ?><?php echo e(hexToRgb(setting('color'))); ?><?php else: ?><?php echo e(hexToRgb('#6366f1')); ?><?php endif; ?>;
            }
        </style>
        <?php echo $__env->make('partials.theme-colors', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    ...
    <body class="min-h-screen bg-white dark:bg-gray-950" x-data="{'sidebarToggle': false, cookiePolicy: localStorage.getItem('cookiePolicy')}" x-bind:class="{ 'false': cookiePolicy }" x-init="$watch('cookiePolicy', val => localStorage.setItem('cookiePolicy', val))">
        ...
        <script src="<?php echo e(asset('static/js/lazysizes.js')); ?>" onerror="console.log('lazysizes not found')"></script>
            ...
            function notificationBell() {
                return {
                    ...
                    async markAsRead(notificationId) {
                        try {
                            const response = await fetch(`<?php echo e(route("admin.notifications.mark-as-read", ":id")); ?>`.replace(':id', notificationId), {
                                method: 'POST',
                                headers: {
                                    'Accept': 'application/json',
                    ...
                    async markAllAsRead() {
                        try {
                            const response = await fetch('<?php echo e(route("admin.notifications.mark-all-as-read")); ?>', {
                                method: 'POST',
                                headers: {
                                    'Accept': 'application/json',
                                    'Content-Type': 'application/json',
...
<?php /**PATH D:\Dev_Projects\Laravel_Projects\taramid\projectfiles\allfilesbackupstaramid\resources\views/layouts/admin.blade.php ENDPATH**/ ?>