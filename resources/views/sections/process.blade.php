...
<section id="process" class="py-24 lg:py-32 bg-white">
    <div class="max-w-6xl mx-auto px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div class="bg-surface rounded-2xl p-7 hover:shadow-card transition-all duration-300 group reveal {{ $index > 0 ? 'reveal-delay-' . $index : '' }}">
                <div class="text-4xl font-display font-bold text-border mb-5 group-hover:text-primary/15 transition-colors">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</div>
                <div class="w-10 h-10 rounded-xl bg-primary/8 flex items-center justify-center mb-5 group-hover:bg-primary/12 transition-colors">
                    @if($step->icon)
                        <div class="w-8 h-8" style="background-color: #2B5F3F; -webkit-mask-image: url('{{ asset('storage/' . $step->icon) }}'); -webkit-mask-size: contain; -webkit-mask-repeat: no-repeat; -webkit-mask-position: center; mask-image: url('{{ asset('storage/' . $step->icon) }}'); mask-size: contain; mask-repeat: no-repeat; mask-position: center;"></div>
                    @else
                        <svg class="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $defaultIcons[$index + 1] ?? $defaultIcons[1] !!}</svg>
                    @endif
                </div>
                <h4 class="font-display text-lg text-ink font-medium mb-2">{{ $step->t('title') }}</h4>
                <p class="text-sm text-ash leading-relaxed">{{ $step->t('description') }}</p>
            </div>
            @endforeach
        </div>
    </div>
