...
@section('content')
    <div class="container-fluid">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{__('Slider')}}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{__('Manage the scrolling slider items on the homepage')}}</p>
            </div>
            <a href="{{route('admin.slider-items.create')}}"
                        ...
                        @forelse($listings as $listing)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($listing->logo)
                                        <img src="{{ asset('storage/' . $listing->logo) }}" alt="" class="h-8 w-8 rounded object-contain bg-gray-100 dark:bg-gray-800">
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $listing->text }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end space-x-3">
                                        <a href="{{route('admin.slider-items.edit', $listing->id)}}"
                                        ...
                                        <form method="POST" action="{{route('admin.slider-items.destroy', $listing->id)}}"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="text-gray-400 dark:text-gray-500">
                                        ...
                                        <p class="text-sm">{{__('No slider items yet.')}}</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
