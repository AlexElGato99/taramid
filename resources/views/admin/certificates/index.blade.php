...
@section('content')
    <div class="container-fluid">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{__('Certificates')}}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{__('Manage the certificates displayed in the Certifications section')}}</p>
            </div>
                        ...
                        @forelse($listings as $listing)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($listing->icon)
                                        <img src="{{ asset('storage/' . $listing->icon) }}" alt="" class="h-8 w-8 rounded object-contain bg-gray-100 dark:bg-gray-800">
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">
                                        {{ $listing->status_label ?: 'Active' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-500 dark:text-gray-400 max-w-xs truncate">{{ $listing->detail_line }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end space-x-3">
                                        <a href="{{route('admin.certificates.edit', $listing->id)}}"
                                        ...
                                        <form method="POST" action="{{route('admin.certificates.destroy', $listing->id)}}"
