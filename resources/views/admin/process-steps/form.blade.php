...
@section('content')
    <div class="max-w-3xl mx-auto w-full">
        ...
        <div class="mb-6">
            <a href="{{ route('admin.process-steps.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 mb-3">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                {{__('Back to Process Steps')}}
            </a>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $config['title'] }}</h1>
        </div>

        @isset($listing)
            <x-admin.lang-select :fields="(new \App\Models\ProcessStep)->translatableFields()"/>
        @endisset

        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm p-6 lg:p-8">
            <form method="POST"
                  action="{{ isset($listing) ? route('admin.process-steps.update', $listing->id) : route('admin.process-steps.store') }}"
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                                  ...
                                  placeholder="{{ model_placeholder($listing ?? null, 'description', __('Step description')) }}">{{ old('description', model_value($listing ?? null, 'description', '')) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{__('Upload an icon image for this step')}}</p>
